...
class LearningNode(Node):
    ...
    def __init__(self):
        ...
        # Env Settings
        self.obs_dim = SENSOR_SECTION + 2 + 2 + 1 + 2 # LIDAR + distance + angle + past action + yaw + current coordinates
        self.act_dim = 2 # v, w
        self.act_limit = 1
        self.v_max = 0.6
        self.v_min = -1 * self.v_max
        self.w_max = 1
        self.w_min = -1 * self.w_max
        ...
        self.pi_lr=1e-3
        self.q_lr=1e-3
        self.start_steps=500
        self.update_after=500
        ...
        self.critic_mem_pre_lstm_hid_sizes=(512,)
        self.critic_mem_lstm_hid_sizes=(512,)
        self.critic_mem_after_lstm_hid_size=(512,)
        self.critic_cur_feature_hid_sizes=(512,)
        self.critic_post_comb_hid_sizes=(512,)
        self.critic_hist_with_past_act=True
        self.actor_mem_pre_lstm_hid_sizes=(512,)
        self.actor_mem_lstm_hid_sizes=(512,)
        self.actor_mem_after_lstm_hid_size=(512,)
        self.actor_cur_feature_hid_sizes=(512,)
        self.actor_post_comb_hid_sizes=(512,)
    ...
    def get_state(self, ranges_arr, past_action):
        ...
        at_list.append(self.yaw)
        at_list.append(self.position.x)
        at_list.append(self.position.y)
    ...
    def get_reward(self, state):
        ...
        dis = np.sqrt((self.position.x - X_GOAL)**2 + (self.position.y - Y_GOAL)**2)
        reward_dis = DIS_START_GOAL - dis
        radians = atan2(Y_GOAL-self.position.y, X_GOAL-self.position.x)
        reward_angle = 1-abs(self.yaw - radians)
        ...
        # else:
        #     self.stop = 0
        # reward = reward_dis + reward_angle
        reward = reward_dis
    ...
    def call_back(self):
        ...
        self.crash = check_crash(ranges_arr, MIN_RANGE)
        self.win = check_win(x, y, X_GOAL, Y_GOAL, MIN_RANGE)
    ...
    def rl_func(self,x,y,yaw,ranges_arr, range_max, angle_max, angle_increment):
        ...
        if self.tstep == 0:
            self.state = self.get_state(ranges_arr, [0] * self.act_dim)
            self.ep_ret = 0
            self.ep_len = 0
            self.previous_action = np.float32([0, 0])
            if self.max_hist_len > 0:
                self.o_buff = np.zeros([self.max_hist_len, self.obs_dim])
                self.a_buff = np.zeros([self.max_hist_len, self.act_dim])
                self.o_buff[0, :] = self.state
                self.o_buff_len = 0
            else:
                self.o_buff = np.zeros([1, self.obs_dim])
                self.a_buff = np.zeros([1, self.act_dim])
                self.o_buff_len = 0
        
        # Randomly draw actions until we get over start steps
        if self.all_n_steps > self.start_steps:
            print("Getting Action From Model...")
            a = self.get_action(self.state, self.o_buff, self.a_buff, self.o_buff_len, self.act_noise, self.device)
        ...
        if self.max_hist_len != 0:
            if self.o_buff_len == self.max_hist_len:
                ...
                self.o_buff[self.max_hist_len - 1] = list(self.state)
                self.a_buff[self.max_hist_len - 1] = list(a)

            else:
                self.o_buff[self.o_buff_len + 1 - 1] = list(self.state)
                self.a_buff[self.o_buff_len + 1 - 1] = list(a)
                self.o_buff_len += 1
        ...
        print(f"Episode Step: {self.tstep}, Total Steps: {self.all_n_steps} ,Reward: {reward}, Total Reward: {self.total_rewards}, Episode Length: {self.ep_len}, Episode Return: {self.ep_ret}")

        # Check maximum time step
        if self.tstep > self.steps_per_epoch:
            self.done = True

        if self.all_n_steps >= self.update_after and self.all_n_steps % self.update_every == 0:
