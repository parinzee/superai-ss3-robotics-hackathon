...
class LearningNode(Node):
    ...
    def get_state(self):
        ...
        """
        Normalized data
        EDIT BY SENMEE 23 FEB 2023
        """
        ra_list = [normalized(ra, 3.5, 0) for ra in ranges_arr]              # Get normalized Lidar
        
        pa_list = []                                    
        pa_list.append(normalized(self.past_action[0], self.v_max, self.v_min))   # Get normalized previous v
        pa_list.append(normalized(self.past_action[1], self.w_max, self.w_min))   # Get normalized previous w

        goal_data = []
        goal_data.append(normalized(self.diff_angle, math.pi, -math.pi))     # Get normalized goal angle
        goal_data.append(normalized(distance, MAX_GOAL_DIS, 0))       # Get normalized goal distance

        merged =  np.array(ra_list + pa_list + goal_data)

        # Remove nan
        merged = np.nan_to_num(merged, posinf=1.0, neginf=0.0)
        return merged
...
class ROS2Env(gym.Env):
    ...
    def __init__(self, x_init = 0, y_init = 0, x_goal = 1.85, y_goal = 0.619, min_range = 0.15, train=True):
        ...
        # EDIT BY SENMEE 23 FEB 2023
        self.action_space = spaces.Box(low=np.array([-1.0, -1.0]), high=np.array([1.0, 1.0]), dtype=np.float64) 

        # Observation Space = Laserscan + Robot Pose + distance + target
        self.observation_space = spaces.Box(shape=(LIDAR_SEGMENT + 3 + 1 + 2,), dtype=np.float64, low=-999.0, high=999.0)
    ...
    def get_reward(self, state):
        ...
        dis_to_goal = get_goal_distance(self.position.x, self.position.y, self.x_goal, self.y_goal)
    ...
    def step(self, action):
        ...
        # EDIT BY SENMEE 23 FEB 2023
        v = action_unnormalized(action[0], V_MAX, V_MIN)
        w = action_unnormalized(action[1], W_MAX, W_MIN)
        
        # Start Running
        self.env2ros.put([v, w])
        ...
        info = {
            'x': self.position.x,
            'y': self.position.y,
            'yaw': next_state[LIDAR_SEGMENT+2],
            'dis': get_goal_distance(*next_state[LIDAR_SEGMENT:LIDAR_SEGMENT+2], self.x_goal, self.y_goal),
            'reward': reward,
        }
